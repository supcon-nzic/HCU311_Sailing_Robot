...
def SavePosition(self,msg):
    latitude = msg.latitude
    longitude = msg.longitude
   # position_save = list(self.nav.latlon_to_utm(msg.latitude, msg.longitude))
...
if __name__ == '__main__':
    ...
    rospy.init_node('apriltags',anonymous=True)
#    rospy.Subscriber('position',NavSatFix, SavePositon)
    

    at_detector = Detector(searchpath=['apriltag-master/lib', 'apriltag-master/lib64'],
                           families='tag36h11',
                           nthreads=1,
                           quad_decimate=1.0,
                           quad_sigma=0.0,
    ...
    videoWriter = cv2.VideoWriter('/root/HCU-311/apriltag/apriltagtest.avi', fourcc, 1.0, (640, 480))
    videoWriter_original = cv2.VideoWriter('/root/HCU-311/apriltag/apriltagtest_original.avi', fourcc, 1.0, (640, 480))
    #rate = rospy.Rate(10)
	...
	frame = frame[:, 0:640, :]
        ...
        tags = at_detector.detect(gray, False, None, None)
	#videoWriter_original.write(frame)
	
	for tag in tags:
            for idx in range(len(tag.corners)):
	        cv2.line(frame, tuple(tag.corners[idx-1, :].astype(int)), tuple(tag.corners[idx, :].astype(int)), (0, 255, 0), 4)
            if len(tags) > 0:
                png_dir = str(num) + '.png'
                print(png_dir)
                cv2.imwrite('/root/HCU-311/apriltag/picture'+png_dir,frame)
                position_file = open('/root/HCU-311/apriltag/position.txt','a+')
                ticks = time.asctime(time.localtime(time.time()))
                data = str(num) + ' ' + ticks + ' ' + str(latitude) + ' ' + str(longitude) + '\n'
