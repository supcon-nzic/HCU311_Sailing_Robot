...
class StationKeeping(taskbase.TaskRunnerBasedTaskBase):
    def __init__(self, nav, waypoint, radius = 5, accept_radius = 15, linger = 30, threshold = 10, waypoint_id = None,
                 kind = 'keep_station', name = '', *args, **kwargs):
        rospy.logwarn("keep_station444444444444444444444")
    ...
    def check_end_condition(self):
        "Are we done yet?"
        yy = self.start_time
        rospy.logwarn('yy'+str(yy))
        tt = time.time
        rospy.logwarn('tt'+str(tt))
        rospy.logwarn('self.start_time is not None and time.time() - self.start_time > self.linger'+str(self.start_time is not None and time.time() - self.start_time > self.linger))
